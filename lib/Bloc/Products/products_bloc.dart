...
class ProductsBloc extends Bloc<ProductsEvent, ProductsState> {
  ...
  Future<void> _onAddNewCategory(
    ...
    try {
      ...
      final data = await categoryController.addNewCategory(
          event.nameCategory, event.descriptionCategory);

      if (data.success)
        emit(SuccessProductsState());
      else
        emit(FailureProductsState(data.error));
    } catch (e) {
  ...
  Future<void> _onAddNewProduct(
    ...
    try {
      ...
      final data = await productController.addNewProduct(event.name,
          event.description, event.price, event.images, event.category);
  ...
  Future<void> _onUpdateStatusProduct(
    ...
    try {
      ...
      if (resp.success)
        ...
        emit(FailureProductsState(resp.error));
    } catch (e) {
      emit(FailureProductsState(e.toString()));
    }
  }
